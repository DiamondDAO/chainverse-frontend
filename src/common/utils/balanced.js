...
exports.matches = function (config) {
  ...
  if (config.source === undefined || config.source === null) throw new Error('Balanced: please provide a "source" property');

  const matchesData = balanced.matchContentsInBetweenBrackets(config.source, config.ignore);

  const newMatchesMap = matchesData?.map(x => {
    const data = config.source.substring(x.index, x.index + x.length)
    return ({
      ...x,
      data
    })
